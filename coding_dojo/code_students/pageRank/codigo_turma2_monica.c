...
float CalculaPageRankVertice(Grafo *grafo,float* page_rank,
  int* out_degree,int vertice,float dumping_factor)
  {
    ...
    for(int i=0;i<grafo->tamanho;i++){
        if(grafo->matrizadj[i][vertice]>0){
          pageRank += page_rank[i]/(float)out_degree[i];
        }
    }

    pageRank=pageRank*dumping_factor+(1-dumping_factor);
  ...
  void CalculaPageRank(Grafo*grafo,float dumping_factor)
  {
    ...
    for(i=0;i<grafo->tamanho;i++)
    {
      vetorPR[i]=1-dumping_factor;
    }

    do
    {
    for(i=0;i<grafo->tamanho;i++){
       vetorPR_atual[i] = CalculaPageRankVertice(grafo, vetorPR, out_degree, i, dumping_factor);
       }
       ...
       for(i=0;i<grafo->tamanho;i++)
       {
         somaDifPR+=fabs(vetorPR_atual[i]-vetorPR[i]);
         vetorPR[i]=vetorPR_atual[i];
       }
