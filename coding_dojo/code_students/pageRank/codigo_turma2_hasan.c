...
{
  /* Gera, para cada vértice, o seu grau
  de saída
  ...
  int cont=0, i=0, j;
  for(i=0;i<grafo->tamanho;i++){
    for(j=0,cont=0;j<grafo->tamanho;j++){
    ...
    for(j=0;j<grafo->tamanho;j++)
      {
          if(grafo->matrizadj[j][vertice] == 1)
            {
              soma+=page_rank[j]/(float)out_degree[j];
            }
      }
pageRank=(1-dumping_factor)+dumping_factor*soma;
...
return pageRank;
  ...
  void CalculaPageRank(Grafo*grafo,float dumping_factor)
  {
    ...
    do
    {
        ...
        {
          vetorPR_atual[k]=CalculaPageRankVertice(grafo, vetorPR, out_degree, k, dumping_factor);

        }

        normalizaVetor(vetorPR_atual, grafo->tamanho);
        for(k=0, somaDifPR = 0; k<grafo ->tamanho; k++)
          somaDifPR+=fabs(vetorPR[k]-vetorPR_atual[k]);

        for(k = 0; k < grafo->tamanho; k++)
          vetorPR[k] = vetorPR_atual[k];
      ...
      printf("Iteração %d Posicao i %d possui page rank de %f\n",iteracao,i,vetorPR[i]);
    }
    iteracao++;
...
/*Testa o pageRank (comente os demais testes para roda-lo*/
	...
	imprimeTopKPageRank(&grafo,5700);
}
